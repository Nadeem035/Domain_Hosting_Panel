<div>
    ...
    <div class="mt-4">
        ...
        <div wire:loading.remove wire:target="search, typeFilter, goToPage, previousPage, nextPage">
            <div class="card overflow-hidden">
                <div class="grid grid-cols-1 gap-x-6 gap-y-1 px-5 py-3 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 sm:grid-cols-[2fr_1.25fr_1fr_1fr_1fr_auto]">
                    <span>Panel</span>
                    <span>Host</span>
                    <span>Type</span>
                    <span>Plans</span>
                ...
                @forelse ($this->panels as $panel)
                    <a href="{{ route('panels.show', $panel) }}" wire:navigate
                        class="table-row grid grid-cols-1 gap-2 px-5 py-4 transition hover:bg-zinc-50/80 sm:grid-cols-[2fr_1.25fr_1fr_1fr_1fr_auto] sm:items-center dark:hover:bg-zinc-800/40">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <span class="min-w-0">
                                <span class="block truncate font-semibold text-zinc-900 dark:text-zinc-100">{{ $panel->name }}</span>
                ...
                @empty
                    <div class="px-6 py-16 text-center">
                        @if ($this->search !== '' || $this->typeFilter !== '')
                            ...
                            <p class="mt-4 text-sm font-medium text-zinc-600 dark:text-zinc-300">No panels match your filters</p>
                            <button wire:click="$set('search', ''); $set('typeFilter', '')" class="btn-ghost mt-2 text-primary-600 dark:text-primary-400">
                                Clear filters
                            </button>
                        @else
                            <x-icon name="panels" class="mx-auto h-10 w-10 text-zinc-300 dark:text-zinc-600" />
                            <p class="mt-4 text-sm font-medium text-zinc-600 dark:text-zinc-300">No panels yet</p>
                            <p class="mt-1 text-xs text-zinc-400 dark:text-zinc-500">Add the servers and control panels you use so you can attach plans and services.</p>
                            <a href="{{ route('panels.create') }}" wire:navigate class="btn-primary mt-4">
                                <x-icon name="plus" class="h-4 w-4" />
            ...
            @if ($this->panels->hasPages())
                <div class="mt-4">{{ $this->panels->links() }}</div>
            @endif
        </div>
    </div>
